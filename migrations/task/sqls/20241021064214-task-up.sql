...
SELECT (SELECT id FROM "USER" WHERE name = '王小明'), id, credit_amount, price
FROM "CREDIT_PACKAGE"
...
INSERT INTO "CREDIT_PURCHASE" (user_id, credit_package_id, purchased_credits, price_paid)
SELECT (SELECT id FROM "USER" WHERE name = '好野人'), id, credit_amount, price
FROM "CREDIT_PACKAGE"
WHERE name = '14 堂組合包方案';
...
INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
SELECT "COACH".id, (SELECT id FROM "SKILL" WHERE name = '瑜伽')
FROM "COACH"
INNER JOIN "USER" ON "USER".id = "COACH".user_id
WHERE "USER".name = '肌肉棒子';

INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
SELECT "COACH".id, (SELECT id FROM "SKILL" WHERE name = '有氧運動')
FROM "COACH"
  INNER JOIN "USER" ON "USER".id = "COACH".user_id
WHERE "USER".name = 'Q太郎';

INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
SELECT "COACH".id, (SELECT id FROM "SKILL" WHERE name = '復健訓練')
FROM "COACH"
  INNER JOIN "USER" ON "USER".id = "COACH".user_id
WHERE "USER".name = 'Q太郎';
...
WHERE user_id = (SELECT id FROM "USER" WHERE "USER".name = '王小明')
AND course_id = (SELECT "COURSE".id FROM "COURSE"
                 INNER JOIN "USER" ON "USER".id = "COURSE".user_id
                 WHERE "USER".name = '李燕容');
...
-- 5-5. 修改：`王小明` 現在已經加入直播室了，請在`COURSE_BOOKING`更新該筆預約資料（請注意，不要更新到已經取消的紀錄）：
    ...
    -- 2. 狀態`status` 設定為上課中
UPDATE "COURSE_BOOKING" SET
  status = '上課中', join_at = '2024-11-25 14:01:59' 
WHERE user_id = (SELECT id FROM "USER" WHERE "USER".name = '王小明')
AND status = '即將授課';
...
FROM "COACH_LINK_SKILL" AS CS
    INNER JOIN "SKILL" AS S ON S.id = CS.skill_id
GROUP BY S.name
ORDER BY COUNT(*) DESC
LIMIT 1;
...
SELECT B.name, COUNT(*) as 銷售數量
FROM "CREDIT_PURCHASE" A
INNER JOIN "CREDIT_PACKAGE" B ON B.id = A.credit_package_id
